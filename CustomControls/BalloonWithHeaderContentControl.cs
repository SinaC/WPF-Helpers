...
{
    // ................. <-- grid with 2 row, header in row 0 and content panel + path in row 1
    // .   Header      .
    // .................
    // . ----|-------- .
    // .| *********** |<--Path
    ...
    public class BalloonWithHeaderContentControl : FrameworkElement
    {
        private readonly System.Globalization.CultureInfo _stringFormatCulture;

        private readonly Grid _grid; // host Header (row 0) and ContentPanel (row 1)
        private readonly SimplePanel _contentPanel; // host Content with a margin added to put Content inside Path
        private readonly List<object> _logicalChildren; // contains Content and Header (only real children)
        ...
        public static readonly DependencyProperty PositionProperty =
            DependencyProperty.Register("Position", typeof(Dock), typeof(BalloonWithHeaderContentControl), new PropertyMetadata(Dock.Top, PositionChanged));
        
        private static void PositionChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            BalloonWithHeaderContentControl control = d as BalloonWithHeaderContentControl;

            if (control == null)
                return;

            // Top: header in row 0 and content in row 1
            // Bottom: content in row 0 and header in row 1
            if (control.Position == Dock.Top)
            {
                UIElement header = control.Header as UIElement;
                if (header != null)
                    Grid.SetRow(header, 0);
                Grid.SetRow(control._path, 1);
                Grid.SetRow(control._contentPanel, 1);
            }
            else if (control.Position == Dock.Bottom)
            {
                UIElement header = control.Header as UIElement;
                if (header != null)
                    Grid.SetRow(header, 1);
                Grid.SetRow(control._path, 0);
                Grid.SetRow(control._contentPanel, 0);
            }

            control.UpdatePathAndHeader();
        ...
        public static readonly DependencyProperty CornerRadiusProperty =
            DependencyProperty.Register(
                "CornerRadius",
                typeof (double),
                typeof (BalloonWithHeaderContentControl),
        ...
        private static void OnHeaderChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (e.OldValue != null)
            {
                ...
                UIElement uiElement = e.OldValue as UIElement;
                if (uiElement != null)
                {
                    control._grid.Children.Remove(uiElement);
            ...
            if (e.NewValue != null)
            {
                ...
                if (uiElement != null)
                {
                    control._grid.Children.Add(uiElement);
        ...
        public BalloonWithHeaderContentControl()
        {
            ...
            // Host _contentPanel, _path and Header
            _grid = new Grid();
            ...
            // Order is important, both controls are on the same grid.row -> display path first then content
            _grid.Children.Add(_path);
            Grid.SetRow(_path, 1);
            _grid.Children.Add(_contentPanel);
            Grid.SetRow(_contentPanel, 1);
            _grid.SizeChanged += PanelOnSizeChanged;
            //
            AddVisualChild(_grid); // only one visual child
        ...
        private void UpdatePathAndHeader()
        {
            // Set Path.Data
            double width = ActualWidth;
            double rowHeight = 0;
            if (Position == Dock.Top)
                rowHeight = _grid.RowDefinitions[0].ActualHeight;
            else if (Position == Dock.Bottom)
                rowHeight = _grid.RowDefinitions[1].ActualHeight;
            double height = ActualHeight - rowHeight - StrokeThickness; // TODO: why substracting StrokeThickness ???
            ...
            // Set Header margin if possible (center of header is above top of arrow)
            FrameworkElement header = Header as FrameworkElement;
            if (header != null)
            {
                header.HorizontalAlignment = HorizontalAlignment.Left;
                double arrowMiddleX = ActualWidth * ArrowPosition;
                double headerWidth = header.ActualWidth;
                double marginX = arrowMiddleX - headerWidth / 2;
                header.Margin = new Thickness(marginX, 0, 0, 0);
            }
        }
